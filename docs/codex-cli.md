...
| 模式 | 安装命令 | 适用场景 |
|------|----------|----------|
| 原生 | `./install.sh --target codex-native` | 只想使用 Codex 官方 `tui.status_line` |
| 增强 | `./install.sh --target codex` | 需要 `5h` / `weekly`、多行 bars、主题、Git diff |
| 增强 + hooks | `./install.sh --target codex --with-hooks` | 需要增强状态栏，且想把实验性 hooks 的最近 Bash 状态接到状态栏 |

`notify bridge` 是可叠加能力，可以和原生或增强模式一起安装。

### 原生模式写入内容

`codex-native` 会把以下配置写入 `~/.codex/config.toml`：
...
Codex 官方还有实验性 hooks，可用于 SessionStart / Tool 前后 / Stop 等生命周期脚本，但它们不是 Claude 那种可直接自定义状态栏渲染的 `statusLine.command`。
...
本项目可以通过 `--with-hooks` 一键安装 sidecar：
...
- 写入 `~/.codex/config.toml` 的 `[features].codex_hooks = true`
- 在 tmux 增强模式下写入 `[statusline].show_hook_segment = true`

当前 sidecar 会把最近一次 hooks 事件写到缓存里，状态栏可显示：
...
> 官方当前限制：`PreToolUse` / `PostToolUse` 的 `matcher` 虽然是正则，但运行时目前只会发出 `Bash`。

### 原生通知 / notify bridge

Codex 顶层还支持 `notify` 外部命令，用来在需要用户注意时把 JSON payload 交给本地脚本处理。本项目提供了 `codex-notify-bridge`，可与原生模式或 tmux 增强模式组合。
...
```toml
notify = ["~/.codex/bin/codex-notify-bridge"]
...
- 读取 Codex 发来的通知 payload，并将最近一次通知写入 `/tmp/codex/statusline-notify-cache.json`
- 在 macOS 上通过 `osascript` 转发桌面通知
- 在 Linux 上通过 `notify-send` 转发桌面通知
- 在 tmux 增强模式下，如果安装时使用 `--with-notify`，会自动开启 `show_notify_segment = true`，在概览行显示 `notify ...`

## 前置依赖

| 工具 | 用途 |
|------|------|
| `tmux` | 终端复用器，承载增强模式底部状态栏 |
| `jq` | 解析 session JSONL（仅增强模式需要） |
...
# 只启用 Codex 原生 tui.status_line
./install.sh --target codex-native

# 启用 Codex tmux 增强模式，并安装实验性 hooks sidecar
./install.sh --target codex --with-hooks

# 给原生或增强模式叠加 notify bridge
./install.sh --target codex-native --with-notify
./install.sh --target codex --with-hooks --with-notify
...
本项目没有单独的升级命令。更新方式就是拉最新代码后，按你当前使用的模式重新执行一次安装脚本。
...
cp scripts/codex_tmux.sh ~/.codex/bin/codex-tmux
cp scripts/codex_statusline.sh ~/.codex/bin/codex-statusline
...
原生模式不需要额外启动器，直接启动 Codex 即可，底部由官方 `tui.status_line` 渲染。
...
增强模式支持两种配置方式，**环境变量优先级高于 config.toml**。

### 环境变量

| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `CODEX_STATUSLINE_SEGMENTS` | (全部) | 逗号分隔的可见段落列表：`model,eff,git,ctx,hook,notify,5h,7d`。未设置则显示全部 |
...
| `CODEX_STATUSLINE_SHOW_HOOK_SEGMENT` | `false` | 显示最近一次 hooks sidecar 状态，如 `hook bash ok` |
| `CODEX_STATUSLINE_HOOK_CACHE_FILE` | `/tmp/codex/statusline-hook-cache.json` | hooks sidecar 写入的状态缓存文件 |
| `CODEX_STATUSLINE_HOOK_TTL` | `600` | hook 缓存可见时长，单位秒 |
| `CODEX_STATUSLINE_SHOW_NOTIFY_SEGMENT` | `false` | 显示最近一次 notify bridge 状态，如 `notify Waiting for approval` |
| `CODEX_STATUSLINE_NOTIFY_CACHE_FILE` | `/tmp/codex/statusline-notify-cache.json` | notify bridge 写入的状态缓存文件 |
| `CODEX_STATUSLINE_NOTIFY_TTL` | `600` | notify 缓存可见时长，单位秒 |
...
在 `~/.codex/config.toml` 中添加项目私有的 `[statusline]` 段落：
...
show_hook_segment = true
show_notify_segment = true
...
| `show_hook_segment` | `true`、`false` | 在概览行或 compact 中显示最近一次 hooks 状态 |
| `show_notify_segment` | `true`、`false` | 在概览行或 compact 中显示最近一次 notify 状态 |
