...
- 5h 速率限制（`rate_limits.primary`）— 用量百分比 + Unix 时间戳重置时间
- 2w 速率限制（`rate_limits.secondary`）— 用量百分比 + Unix 时间戳重置时间
...
| **5h** | 5 小时速率限制用量百分比 + 重置时间 | `5h 14% 13:30` |
| **2w** | 2 周速率限制用量百分比 + 重置时间 | `2w 4% 03 18 09:00` |
...
在 `bars` 布局中，概览行先裁剪；5h / 2w 进度条行会先缩短时间文本，最后才缩小进度条宽度。
...
<details>
<summary><strong>状态栏显示 <code>5h -</code> / <code>2w -</code>，没有用量数据？</strong></summary>

说明找不到有效的 session JSONL 文件，或文件中没有 `token_count` 事件。确认 Codex CLI 已运行过至少一次对话。检查 `~/.codex/sessions/` 目录是否存在 `.jsonl` 文件。
</details>
